...
The payment engine records **$335,668.57**, while provider records total **$134,784.52**. The signed principal difference (`provider - engine`) is **-$200,884.05**; gross absolute differences total **$200,956.79**. Every monetary difference is assigned a cause, and the summary reconciles to the detail with a **$0.00** residual.
...
- **Adyen:** a chargeback is missing from the engine (**-$64.79**), two sale amounts differ (**-$40.36**), and one engine-settled payment was declined by Adyen (**-$10.90**). One June engine event settled at Adyen on July 2, moving **$10.88** to the next provider period while the engine recognized **$10.79** in June. One provider-only sale contributes **+$10.86**; an earlier engine FX date causes the remaining **+$0.03** difference.
...
5. Track the Adyen cutoff item into July and investigate the remaining **$0.09** cross-period FX valuation difference.
...
- PayPal EU dates use `DD/MM/YYYY` when that date falls within the export interval. Three outlier strings use `MM/DD/YYYY` because only that interpretation falls within the interval.
- The smaller FX differences come from an earlier engine FX date (Adyen and PayPal EU) or a later rate revision (dLocal).
- One-cent differences with equal local amounts and FX rates are classified as `rounding_difference`. This is a working tolerance because provider rounding rules were not supplied.
